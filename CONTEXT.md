...
- **PaymentOutcome**: The result of a payment attempt that mutates a `CheckoutSession`. For credit-card checkout, a successful Payfirma approval records `paid_at` while intentionally leaving `custom_attrs.checkoutCompleted` unset until the customer completes the order.
